...
if __name__ == "__main__":
	...
	with Radar(args.configuration) as device:
    ...
    os.mkdir(dst / "Radar_00")
    os.mkdir(dst / "Labels_00")
    
    shutil.copy(dst / "format.version", dst / "Labels_00")
    shutil.move(dst / "format.version", dst / "Radar_00")

    shutil.copy(dst / "meta.json", dst / "Labels_00")
    shutil.copy(dst / "meta.json", dst / "Radar_00")
    
    shutil.copy(dst / "radar_timestamp.csv", dst / "Labels_00/label_timestamp.csv")
    
    shutil.move(dst / "config.json", dst / "Radar_00")
    shutil.move(dst / "radar.npy", dst / "Radar_00")
    shutil.move(dst / "radar_timestamp.csv", dst / "Radar_00")

    
    raw_data = np.load(dst / "Radar_00/radar.npy")
    raw_data = raw_data[:, [1, 0]]
    np.save(dst / "Radar_00/radar.npy", raw_data)

    _, __, result = fft(raw_data, gauss)

    np.save(dst / "Radar_00/target", result)
